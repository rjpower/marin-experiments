...
class GrugTrainerConfig:
    ...
    # Tensor (model) parallelism over the intra-slice devices. On a single 8xH100 node
    # (1 process), `data` = local_device_count / (expert_axis_size * model_axis_size).
...
def _run_grug_local(config: GrugRunConfig) -> None:
    ...
    with set_mesh(mesh):
        ...
        # Weights-only init from a PRIOR run's checkpoint (e.g. the SFT cooldown initializing from
        # the pretrain checkpoint). Only when this run has no checkpoint of its own yet (step==0) --
        # so a cooldown that gets preempted still resumes its OWN full state (the restore above)
        # rather than re-initializing. We graft ONLY the model params (and EMA) and keep step=0 +
        # a fresh optimizer, so the cooldown runs its own short WSD-decay schedule from step 0 and
        # the data loader (iter_from_step) starts at the beginning of the SFT mixture, not skipped
        # ~pretrain_steps in. `trainer.initialize_from` is plumbed from SP_INIT_FROM in launch.py;
        # it is None for normal pretraining, so this block is a no-op there.
        if int(state.step) == 0 and getattr(trainer, "initialize_from", None):
            init_src = trainer.initialize_from
            logger.info("Initializing model weights from prior checkpoint: %s", init_src)
            loaded = restore_grug_state_from_checkpoint(
                state,
                checkpoint_search_paths=[init_src],
            ...
            state = dataclasses.replace(
                ...
                ema_params=(loaded.params if state.ema_params is not None else None),
            )
            logger.info("Weights initialized (step=0, fresh optimizer) for cooldown/finetune.")
