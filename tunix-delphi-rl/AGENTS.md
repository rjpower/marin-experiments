...
| `micropython.py` | A purely-functional, sandboxed, bounded tree-walking interpreter for a Python subset (`run(src) -> ExecResult`, never raises) — the execution environment + verifier for the **agentic coding** follow-on (issue #7, `REPORT.md` §10). 90 tests in `test_micropython.py`. |
| `coding_tasks.py` | The **held-out** 50-task coding ladder (5 tiers × 10) — `(prompt, reference solution, gold stdout)`; the eval set for "how far can the base LM get". |
| `coding_env.py` | The coding RL env: 41 **parameterized task families** (anti-hardcode, like CALC random operands), the `CODE_FEWSHOT` format prefix (shared SFT+RL), the `END`-sentinel program parser, the dense reward + solve/ran_ok/has_code metric, `code_segments` (execution-format SFT), `build_code_dataset`, and the greedy `evaluate_tasks` eval-on-50 (answer hints stripped). |
| `train_coding.py` / `launch_coding.py` | SFT warm-up → **Dr.GRPO** RL driver (`DrGRPOLearner`/`DrGRPOConfig`) + iris entrypoint (`CODING_TIERS / CODING_SFT_STEPS / CODING_STEPS / ...`). **Key result: SFT alone gets 50/50; Dr.GRPO is marginal here — the target behavior (writing a program) is fully demonstrable by SFT, unlike the §9 tool-result copy that RL must amplify.** |
